...
def clockin_clockout(request):
	if request.method == 'POST':
		if 'btnform1' in request.POST:
			if Attendance.objects.filter(attender=str(request.user), date = datetime.utcnow()  + timedelta(hours=5, minutes=30), is_present = False).exists():
				messages.error(request, "You are marked as absent by admin today, you can't clock-in")
			elif not Attendance.objects.filter(attender=str(request.user),date = datetime.utcnow()  + timedelta(hours=5, minutes=30), clockin__isnull=False).exists():
			...
			if not Attendance.objects.filter(attender=str(request.user),date = datetime.utcnow()  + timedelta(hours=5, minutes=30), clockin__isnull=False).exists():
				messages.error(request, 'You must clockin to clockout')
			elif not Attendance.objects.filter(attender=str(request.user),date = datetime.utcnow() + timedelta(hours=5, minutes=30), clockout__isnull=False).exists():
